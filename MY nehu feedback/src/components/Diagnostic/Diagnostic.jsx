...
export function Diagnostic({ submitted, name, setName, dept, setDept, yearOfStudy, setYearOfStudy, usageFreq, setUsageFreq, selectedReasons, toggleReason, reasonDetails, setReasonDetails, handleFormSubmit, resetForm, setTab }) {
  return (
    <CyberCard style={{ padding: '2rem' }}>
      {submitted ? <SubmissionSuccess name={name} onReset={resetForm} onViewAnalytics={() => setTab('analytics')} /> : (
        <form onSubmit={handleFormSubmit}>
          ...
          <StudentDetails name={name} setName={setName} dept={dept} setDept={setDept} yearOfStudy={yearOfStudy} setYearOfStudy={setYearOfStudy} usageFreq={usageFreq} setUsageFreq={setUsageFreq} />
